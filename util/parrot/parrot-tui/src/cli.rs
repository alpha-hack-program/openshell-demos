...
/// parrot — a friendlier terminal frontend for streaming agent CLIs
/// (Claude Code, Codex) running inside OpenShell sandboxes.
///
/// By default, parrot opens an interactive multi-turn session: type a
/// prompt, press Enter, watch it stream, then type the next one. Pass
/// `--prompt` for a one-shot run instead.
#[derive(Debug, Parser)]
#[command(name = "parrot", version)]
pub struct Cli {
    ...
    /// Run a single prompt and exit, instead of an interactive session.
