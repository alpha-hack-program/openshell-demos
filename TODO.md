...
- [ ] Add `--binary /usr/bin/curl` to the `openshell sandbox create` call instead of requiring a separate `openshell policy update` afterwards — the binary permission can be set at creation time, removing an extra step from the guide
