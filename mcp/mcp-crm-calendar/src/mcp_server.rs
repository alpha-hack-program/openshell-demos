...
mod common;
use common::crm_calendar_service::CrmCalendarService;
...
async fn main() -> anyhow::Result<()> {
    ...
    let pool = PgPoolOptions::new()
        ...
        .await?;
    assert_schema_ready(&pool, &["meetings"]).await?;
    tracing::info!("database schema verified");

    let bind_address = std::env::var("BIND_ADDRESS").unwrap_or_else(|_| BIND_ADDRESS.to_string());
    tracing::info!(
        "iniciando mcp-crm-calendar (streamable-http) en {}",
        bind_address
    );

    let service = StreamableHttpService::new(
        move || Ok(CrmCalendarService::new(pool.clone())),
